...
namespace ReadFile
{
    ...
    {
        /// <summary>
        /// Лекция 6. Задание 4. Считайте файл различными способами. Смотрите “Пример записи файла различными
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("FileStream. Milliseconds:{0}", FileStreamSampleWrite("C:\\temp\\bigdata0.bin", size));
            Console.WriteLine("BinaryStream. Milliseconds:{0}", BinaryStreamSampleWrite("C:\\temp\\bigdata1.bin", size));
            Console.WriteLine("StreamWriter. Milliseconds:{0}", StreamWriterSampleWrite("C:\\temp\\bigdata2.bin", size));
            Console.WriteLine("BufferedStream. Milliseconds:{0}", BufferedStreamSampleWrite("C:\\temp\\bigdata3.bin", size));

            Console.WriteLine("Сейчас под капотом нашей колымаги произойдет магия, которую мы Вам не покажем :)");
            byte[] bytesFromFileStream = FileStreamSampleRead("C:\\temp\\bigdata0.bin");
            int[] integersFromBinatyStream = BinaryStreamSampleRead("C:\\temp\\bigdata1.bin");
            string stringFromSreamReader = StreamReaderSample("C:\\temp\\bigdata2.bin");
            byte[] bytesFromBufferedStream = BufferedStreamSampleRead("C:\\temp\\bigdata3.bin");

            Console.WriteLine($"{bytesFromFileStream}");

            Console.ReadKey();
        }

        // Эксперимент с добавлением summary с указанием параметров
